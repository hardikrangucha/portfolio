...
const HeroSection = memo(() => {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      // Animate sequentially with will-change hint
      tl.fromTo(
        titleRef.current,
        { opacity: 0, y: 50, filter: "blur(10px)", willChange: "opacity, transform" },
        { opacity: 1, y: 0, filter: "blur(0px)", duration: 1, ease: "power2.out" }
      )
        .fromTo(
          subtitleRef.current,
          { opacity: 0, y: 40, filter: "blur(10px)", willChange: "opacity, transform" },
          { opacity: 1, y: 0, filter: "blur(0px)", duration: 0.8, ease: "power2.out" },
          "-=0.6"
        )
        .fromTo(
          ctaRef.current,
          { opacity: 0, y: 30, filter: "blur(10px)", willChange: "opacity, transform" },
          { opacity: 1, y: 0, filter: "blur(0px)", duration: 0.8, ease: "power2.out" },
          "-=0.5"
        )
        .fromTo(
          splineRef.current,
          { opacity: 0, x: 80, scale: 0.9, willChange: "opacity, transform" },
      ...
      <div className="container mx-auto px-6 relative z-10">
        <div className="flex flex-col items-center justify-center min-h-screen text-center">
          <div className="space-y-12 max-w-4xl">
            <h1 ref={titleRef} className="text-4xl md:text-6xl lg:text-7xl xl:text-8xl font-bold leading-tight">
              Hi, I'm <span className="text-primary">Hardik</span>
            </h1>

            <div ref={subtitleRef} className="text-xl md:text-2xl lg:text-3xl text-muted-foreground max-w-3xl mx-auto leading-relaxed">
          ...
          <div className="absolute bottom-20 left-1/2 transform -translate-x-1/2">
            <div className="grid grid-cols-3 gap-8 max-w-md">
              {[
                ...
                <div key={stat.label} className="text-center transition-transform duration-500 ease-out hover:scale-110">
